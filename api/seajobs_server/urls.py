...
def update_profile_company(request, email: str, password: str, website: str, mobile_phone: str, country: str, city: str, address: str):
    ...
    try:
        ...
        if not address:
            raise ValueError("Address must be set")
        if request.auth["oner"] != email and (query_db(f"SELECT email FROM companies WHERE email='{email}' LIMIT 1", one=True) or query_db(f"SELECT email FROM users WHERE email='{email}' LIMIT 1", one=True)):
